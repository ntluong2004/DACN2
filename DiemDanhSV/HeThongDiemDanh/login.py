...
class Login_Window:
    def __init__(self, root):
        ...
        title = Label(login_frame, text="Đăng nhập ", font=(
            "times new roman", 30, "bold"), bg="white", fg="#08A3D2").place(x=250, y=40)

        email = Label(login_frame, text="Email", font=("times new roman", 18, "bold"), bg="white",
                      fg="gray").place(x=250, y=130)
        self.txtuser = ttk.Entry(
            login_frame, textvariable=self.var_email, font=("times new roman", 15))
        ...
        self.txtpass = ttk.Entry(login_frame, textvariable=self.var_password, font=(
            "times new roman", 15), background="black", show="*")
        ...
        # =============clock============
        self.lbl = Label(self.root, bg="#081923", text=today, font=(
            "Book Antiqua", 20, "bold"), compound=BOTTOM, fg="white", bd=0)
    ...
    def clock_image(self, hr, min_, sec_):
        ...
        draw.line((200, 200, 200 + 50 * sin(radians(hr)), 200 -
                  50 * cos(radians(hr))), fill="#DF005E", width=4)

        draw.line((200, 200, 200+80*sin(radians(min_)), 200 -
                  80*cos(radians(min_))), fill="white", width=3)

        draw.line((200, 200, 200+100*sin(radians(sec_)), 200 -
        ...
        if self.txtuser.get() == "" or self.txtpass.get() == "":
            ...
            if row == None:
                messagebox.showerror(
                    "Lỗi", "Sai tên đăng nhập, mật khẩu hoặc quyền đăng nhập")
            else:
                new_print(str(0))
                # # self.root.destroy()
                # # import home
                self.reset()
                messagebox.showinfo(
                    "Thông báo", "Bạn đã đăng nhập thành công với quyền Admin")
                self.new_window = Toplevel(self.root)
                self.app = Face_Recognition_System(self.new_window)
            conn.commit()
            conn.close()
        else:
            conn = mysql.connector.connect(host='localhost', user='tienluong', password='1234', database='face_recognizer',
                                           port='3306')
            my_cursor = conn.cursor()
