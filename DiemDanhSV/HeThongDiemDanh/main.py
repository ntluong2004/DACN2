...
class Face_Recognition_System:
    def __init__(self, root):
        ...
        self.lblemail = Label(self.root, text=self.account, font=(
            "yu gothic ui", 12, "bold"), bg="white", fg="green")
        ...
        b1 = Button(self.root, image=self.photoimglogout,
                    cursor="hand2", command=self.exit, borderwidth=0, bg="white")
    ...
    def exit(self):
        Exit = messagebox.askyesno(
            "Đăng xuất", "Bạn có chắc chắn muốn đăng xuất không?", parent=self.root)
        if (Exit > 0):
            self.root.destroy()
        else:
            if not Exit:
    ...
    def reset_pass(self):
        if self.changePass_entry.get == "Select":
            messagebox.showerror(
                "Error", "Hãy chọn câu hỏi bảo mật", parent=self.root2)
        elif self.answerLabel_entry.get() == "":
            messagebox.showerror(
                "Error", "Hãy nhập câu trả lời", parent=self.root2)
        elif self.passLabel_entry.get() == "":
            messagebox.showerror(
                "Error", "Hãy nhập mật khẩu mới", parent=self.root2)
        else:
            ...
            if row == None:
                messagebox.showerror(
                    "Error", "Sai câu hỏi bảo mật hoặc đáp án ", parent=self.root2)
            else:
                my_cursor.execute("update teacher set Password=%s where Teacher_id=%s",
                                  (self.passLabel_entry.get(), str(value_from_p1),))
                conn.commit()
                conn.close()
                messagebox.showinfo(
                    "Thông báo", "Đổi mật khẩu thành công", parent=self.root2)
    ...
    def change_pass(self):
        ...
        l = Label(self.root2, text="Đổi mật khẩu", font=(
        ...
        self.changePass_entry["values"] = (
            "Select", "Bạn thích ăn gì", "Sở thích của bạn", "Chữ số bạn thích")
        ...
        btn = Button(self.root2, text="Đổi mật khẩu", font=(
            "times new roman", 12, "bold"), fg="white", bg="darkblue", command=self.reset_pass)
