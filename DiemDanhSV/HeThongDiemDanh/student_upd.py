...
class Student:
    def __init__(self, root):
        ...
        lbl = Label(self.root, font=("yu gothic ui", 13, "bold"),
                    bg="white", fg="black")
        lbl.place(x=80, y=35, width=100, height=20)
        time()
        lbl1 = Label(self.root, text=today, font=(
            "yu gothic ui", 13, "bold"), bg="white", fg="black")
        ...
        # left_label
        self.getNextid()
        Left_frame = LabelFrame(main_frame, bd=2, bg="white", font=(
        ...
        # department
        dep_label = Label(current_course_frame, text="Chuyên ngành", font=(
        ...
        dep_combo["values"] = ("Chọn chuyên ngành", "Điện tử viễn thông",
                               "IT", "Cơ khí", "Điện", "Kế toán", "Tự động hóa")
        dep_combo.current(0)
        dep_combo.grid(row=0, column=1, padx=2, pady=10, sticky=W)

        # course
        course_label = Label(current_course_frame, text="Hệ đào tạo", font=(
            "times new roman", 13, "bold"), bg="white")
        ...
        # year
        year_label = Label(current_course_frame, text="Năm học", font=(
            "times new roman", 13, "bold"), bg="white")
        ...
        year_combo = ttk.Combobox(current_course_frame, textvariable=self.var_year, font=("times new roman", 13, "bold"), state="readonly",
                                  width=20)
        year_combo["values"] = ("Chọn năm học", "2020-21",
                                "2021-22", "2022-23", "2023-24")
        year_combo.current(0)
        year_combo.grid(row=1, column=1, padx=2, pady=10, sticky=W)

        # semester
        semester_label = Label(current_course_frame, text="Học kì", font=(
            "times new roman", 13, "bold"), bg="white")
        ...
        # student_id
        studentID_label = Label(class_student_frame, text="ID Sinh Viên:", font=(
            "times new roman", 13, "bold"), bg="white")
        studentID_label.grid(row=0, column=0, padx=10, pady=10, sticky=W)

        studentID_entry = ttk.Entry(class_student_frame, width=20, textvariable=self.var_std_id, font=(
            "times new roman", 13, "bold"), state="disabled")
        ...
        studentName_entry = ttk.Entry(
            class_student_frame, width=20, textvariable=self.var_std_name, font=("times new roman", 13, "bold"))
        ...
        class_div_entry = ttk.Entry(class_student_frame, width=20, textvariable=self.var_div, font=(
        ...
        roll_no_entry = ttk.Entry(class_student_frame, width=20, textvariable=self.var_roll, font=(
            "times new roman", 13, "bold"))
        ...
        email_entry = ttk.Entry(class_student_frame, width=20, textvariable=self.var_email, font=(
            "times new roman", 13, "bold"))
        ...
        phone_entry = ttk.Entry(class_student_frame, width=20, textvariable=self.var_phone, font=(
            "times new roman", 13, "bold"))
        ...
        radionbtn1 = ttk.Radiobutton(
            class_student_frame, variable=self.var_radio1, text="Có ảnh", value="Yes")
        radionbtn1.grid(row=6, column=0)

        radionbtn2 = ttk.Radiobutton(
            class_student_frame, variable=self.var_radio1, text="Không ảnh", value="No")
        ...
        save_btn = Button(btn_frame, text="Thêm", command=self.add_data, font=(
            "times new roman", 13, "bold"), bg="#38a6f0", fg="white", width=17)
        save_btn.grid(row=0, column=0)

        update_btn = Button(btn_frame, text="Sửa", command=self.update_data, font=(
            "times new roman", 13, "bold"), bg="#38a6f0", fg="white", width=17)
        update_btn.grid(row=0, column=1)

        delete_btn = Button(btn_frame, text="Xóa", command=self.delete_data, font=(
            "times new roman", 13, "bold"), bg="#38a6f0", fg="white", width=17)
        delete_btn.grid(row=0, column=2)

        reset_btn = Button(btn_frame, text="Làm mới", command=self.reset_data, font=(
            "times new roman", 13, "bold"), bg="#38a6f0", fg="white", width=17)
        ...
        search_combo["values"] = (
            "ID Sinh viên", "Tên sinh viên", "Lớp biên chế")
        search_combo.current(0)
        search_combo.grid(row=0, column=1, padx=2, pady=10, sticky=W)

        self.var_search = StringVar()
        search_entry = ttk.Entry(search_frame, width=15, font=(
            "times new roman", 13, "bold"), textvariable=self.var_search)
        search_entry.grid(row=0, column=2, padx=10, pady=5, sticky=W)

        search_btn = Button(search_frame, text="Tìm kiếm", font=(
            "times new roman", 12, "bold"), bg="#38a6f0", fg="white", width=12, command=self.search_data)
        ...
        self.student_table = ttk.Treeview(table_frame, column=("id", "dep", "course", "year", "sem", "name", "div", "roll",
                                          "gender", "dob", "email", "phone", "address", "photo"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
        ...
        self.student_table.bind(
            "<ButtonRelease>", self.get_cursor)  # click datagrid
        ...
        # table_frame
        tablestd_frame = Frame(Underright_frame, bd=2,
    ...
    def add_data(self):
        ...
        if self.var_dep.get() == "Chọn chuyên ngành" or self.var_std_name.get() == "" or self.var_std_id.get() == "" or self.var_div.get() == "":
            ...
            messagebox.showerror(
                "Error", "Tên lớp học không tồn tại ! Vui lòng kiểm tra lại", parent=self.root)
        else:
            try:
                conn = mysql.connector.connect(
                    host='localhost', user='root', password='1234', database='face_recognizer', port='3306')
                ...
                conn.close()
                messagebox.showinfo(
                    "Thành công", "Thêm thông tin sinh viên thành công", parent=self.root)
        ...
        if self.var_dep.get() == "Chọn chuyên ngành" or self.var_std_name.get() == "" or self.var_std_id.get() == "":
            ...
            try:
                Update = messagebox.askyesno(
                    "Update", "Bạn có muốn cập nhật thông tin sinh viên này không?", parent=self.root)
                if Update > 0:
                    conn = mysql.connector.connect(
                        host='localhost', user='root', password='1234', database='face_recognizer', port='3306')
                    my_cursor = conn.cursor()
                    ...
                    if not Update:
                        return
                messagebox.showinfo(
                    "Thành công", "Cập nhật thông tin sinh viên thành công", parent=self.root)
                conn.commit()
                self.fetch_data()
                self.reset_data()
                conn.close()
                ...
                messagebox.showerror(
                    "Lỗi", f"Due To:{str(es)}", parent=self.root)

    # Delete Function
    def delete_data(self):
        if self.var_std_id.get() == "":
            messagebox.showerror(
                "Lỗi", "Không được bỏ trống ID sinh viên", parent=self.root)
        else:
            try:
                delete = messagebox.askyesno(
                    "Xoá sinh viên", "Bạn có muốn xóa sinh viên này?", parent=self.root)
                if delete > 0:
                    conn = mysql.connector.connect(
                        host='localhost', user='root', password='1234', database='face_recognizer', port='3306')
                    my_cursor = conn.cursor()
                ...
                conn.close()
                messagebox.showinfo(
                    "Xóa", "Xóa sinh viên thành công", parent=self.root)
            except Exception as es:
                messagebox.showerror(
                    "Lỗi", f"Due To:{str(es)}", parent=self.root)
    ...
    def search_data(self):
        if self.var_com_search.get() == "" or self.var_search.get() == "":
            messagebox.showerror(
                "Lỗi !", "Vui lòng nhập thông tin đầy đủ", parent=self.root)
            ...
            try:
                ...
                if (len(data) != 0):
                    ...
                    for i in data:
                        self.student_table.insert("", END, values=i)
                    messagebox.showinfo(
                        "Thông báo", "Có " + str(len(data)) + " bản ghi thỏa mãn điều kiện", parent=self.root)
                    conn.commit()
                else:
                    self.student_table.delete(
                        *self.student_table.get_children())
                    messagebox.showinfo(
                        "Thông báo", " Không có bản ghi nào thỏa mãn điều kiện", parent=self.root)
                conn.close()
            except Exception as es:
                messagebox.showerror(
                    "Lỗi", f"Due To:{str(es)}", parent=self.root)
            ...
            try:
                ...
                while True:
                    ...
                    if face_cropped(my_frame) is not None:
                        ...
                        face = cv2.cvtColor(face_cropped(
                            my_frame), cv2.COLOR_BGR2GRAY)
                        fill_name_path = "data/user." + \
                ...
                messagebox.showinfo(
                    "Kết quả", "Tạo dữ liệu khuôn mặt thành công", parent=self.root)
            except Exception as es:
                messagebox.showerror(
                    "Lỗi", f"Due To:{str(es)}", parent=self.root)
    ...
    def train_classifier(self):
        ...
        cv2.destroyAllWindows()
        messagebox.showinfo(
            "Kết quả", "Training dataset Completed", parent=self.root)
    ...
    def add_Classdata(self):
        ...
        if self.var_class.get() == "" or self.var_nameclass.get() == "":
            ...
            messagebox.showerror(
                "Error", "Class đã tồn tại! Vui lòng kiểm tra lại", parent=self.root)
        else:
            try:
                conn = mysql.connector.connect(host='localhost', user='root', password='1234',
                                               database='face_recognizer', port='3306')
    ...
    def update_Classdata(self):
        if self.var_class == "" or self.var_nameclass.get() == "":
            ...
            try:
                Update = messagebox.askyesno(
                    "Update", "Bạn có muốn cập nhật bản ghi này không?", parent=self.root)
                if Update > 0:
                    conn = mysql.connector.connect(host='localhost', user='root', password='1234',
                                                   database='face_recognizer', port='3306')
                    my_cursor = conn.cursor()
    ...
    def delete_Classdata(self):
        if self.var_class == "" or self.var_nameclass.get() == "":
            ...
            try:
                ...
                conn.close()
                messagebox.showinfo(
                    "Xóa", "Xóa bản ghi thành công", parent=self.root)
                self.reset_Classdata()
                self.fetch_Classdata()
            except Exception as es:
                messagebox.showerror(
