...
class Subject:
    def __init__(self, root):
        ...
        self.var_teachersub.trace(
            "w", lambda name, index, mode, var_teachersub=self.var_teachersub: self.callback())
        ...
        # subject
        self.var_subsub.trace("w", lambda name, index, mode,
                              var_subsub=self.var_subsub: self.callsubtc())
        ...
        # student
        self.var_studentsub.trace("w", lambda name, index, mode,
                                  var_studentsub=self.var_studentsub: self.callstudent())
        ...
        self.var_subsubst.trace(
            "w", lambda name, index, mode, var_subsubst=self.var_subsubst: self.callstSub())
    ...
    def add_data(self):
        ...
        # =========check subject============
        my_cursor.execute(
            "select Subject_name from `subject` where Class=%s", (self.var_subclass.get(),))
        ...
        if self.var_subid.get() == "Select" or self.var_subclass.get() == "" or self.var_subname.get() == "":
            ...
            messagebox.showerror(
                "Error", "Lớp học đã có môn học này! Vui lòng kiểm tra lại", parent=self.root)
            ...
            messagebox.showerror(
                "Error", "Đã tồn tại mã môn học ! Vui lòng kiểm tra lại", parent=self.root)
        else:
            try:
                conn = mysql.connector.connect(
            ...
            try:
                ...
                if Update > 0:
                    ...
                    if not Update:
                        return
                messagebox.showinfo(
                    "Thành công", "Cập nhật thông tin môn học thành công", parent=self.root)
                conn.commit()
                self.fetch_data()
                conn.close()
            except Exception as es:
    ...
    def delete_data(self):
        if self.var_subid == "":
            messagebox.showerror(
                "Lỗi", "Không được bỏ trống ID ", parent=self.root)
        else:
            try:
    ...
    def search_data(self):
        if self.var_com_search.get() == "" or self.var_search.get() == "":
            ...
            try:
                ...
                my_cursor.execute("select * from subject where "+str(
                ...
                if (len(data) != 0):
                    ...
                    for i in data:
                        self.AttendanceReportTable.insert("", END, values=i)
                    messagebox.showinfo(
                        "Thông báo", "Có "+str(len(data))+" bản ghi thỏa mãn điều kiện")
                    conn.commit()
                else:
                    self.AttendanceReportTable.delete(
                        *self.AttendanceReportTable.get_children())
    ...
    def callback(self):
        ...
        if (self.var_teachersub.get() not in arrayTeacher):
            ...
            my_cursor = conn.cursor()
            my_cursor.execute(
                "select Name from `teacher` where Teacher_id=%s", (self.var_teachersub.get(),))
    ...
    def callsubtc(self):
        ...
        if (self.var_subsub.get() not in arrayTeacher):
            ...
            my_cursor = conn.cursor()
            my_cursor.execute(
                "select Subject_name from `subject` where Subject_id=%s", (self.var_subsub.get(),))
            ckteacher = my_cursor.fetchone()
    ...
    def add_Tcdata(self):
        ...
        # =========check subject============
        my_cursor.execute(
            "select Subject_id from teacher_has_subject where Teacher_id=%s", (self.var_teachersub.get(),))
        ckSubject = my_cursor.fetchall()
        arraySub = []
        for chk in ckSubject:
            arraySub.append(str(chk[0]))
        ...
        my_cursor.execute(
            "select COUNT(Teacher_id) from `teacher_has_subject` where Subject_id=%s", (self.var_subsub.get(),))
        ...
        if self.var_teachersub.get() == "" or self.var_subsub.get() == "":
            ...
            messagebox.showerror(
                "Error", "Giảng viên đã có môn học này! Vui lòng kiểm tra lại", parent=self.root)
        elif (ckTeacher_has1sub[0] > 0):
            messagebox.showerror(
                "Error", "Môn học này đã có giảng viên! Vui lòng kiểm tra lại", parent=self.root)
        elif (self.var_teachersub.get() not in arrayTeacher):
            messagebox.showerror(
                "Error", "ID Giảng viên không tồn tại! Vui lòng kiểm tra lại", parent=self.root)
        elif (self.var_subsub.get() not in arraySubject):
            messagebox.showerror(
                "Error", "ID Môn học không tồn tại! Vui lòng kiểm tra lại", parent=self.root)
        else:
            try:
                conn = mysql.connector.connect(
                    host='localhost', user='root', password='1234', database='face_recognizer', port='3306')
                ...
                conn.close()
                messagebox.showinfo(
                    "Thành công", "Thêm thông tin môn học cho giảng viên thành công", parent=self.root)
            except Exception as es:
                messagebox.showerror(
                    "Error", f"Due To:{str(es)}", parent=self.root)
            ...
            try:
                ...
                if Update > 0:
                    ...
                    if not Update:
                        return
                messagebox.showinfo(
                    "Thành công", "Cập nhật thông tin môn học GV thành công", parent=self.root)
                conn.commit()
                self.reset_Tcdata()
                self.fetch_Tcdata()
                conn.close()
    ...
    def search_Tcdata(self):
        if self.var_com_searchtc.get() == "" or self.var_searchtc.get() == "":
            ...
            try:
                ...
                my_cursor.execute("select * from teacher_has_subject where "+str(
                    self.var_com_searchtc.get())+" Like '%"+str(self.var_searchtc.get())+"%'")
                data = my_cursor.fetchall()
                if (len(data) != 0):
                    self.TeacherTable.delete(*self.TeacherTable.get_children())
                    for i in data:
    ...
    def callstudent(self):
        ...
        if (self.var_studentsub.get() not in arrayStudent):
            ...
            my_cursor = conn.cursor()
            my_cursor.execute(
                "select Name from `student` where Student_id=%s", (self.var_studentsub.get(),))
    ...
    def callstSub(self):
        ...
        if (self.var_subsubst.get() not in arraySubject):
            ...
            my_cursor = conn.cursor()
            my_cursor.execute(
                "select Subject_name from `subject` where Subject_id=%s", (self.var_subsubst.get(),))
    ...
    def add_Stddata(self):
        ...
        if self.var_studentsub == "" or self.var_subsubst.get() == "":
            ...
            messagebox.showerror(
                "Error", "Sinh viên đã có môn học này! Vui lòng kiểm tra lại", parent=self.root)
        elif (self.var_studentsub.get() not in arrayStudent):
            messagebox.showerror(
                "Error", "ID Sinh viên không tồn tại! Vui lòng kiểm tra lại", parent=self.root)
            ...
            try:
                ...
                if Update > 0:
                    ...
                    if not Update:
                        return
                messagebox.showinfo(
                    "Thành công", "Cập nhật thông tin môn học Sinh viên thành công", parent=self.root)
                conn.commit()
                self.reset_Stddata()
                self.fetch_Stddata()
                conn.close()
